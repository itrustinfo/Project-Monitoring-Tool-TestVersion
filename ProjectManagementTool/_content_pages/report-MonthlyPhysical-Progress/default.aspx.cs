...
vAxes: {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (rdSelect.SelectedValue == "0")
                {
                    ...
                    LoadTabularData();
                    headingTb.InnerHtml = "Monthly Physical Progress Report - " + DDlProject.SelectedItem.Text + " (" + DDLWorkPackage.SelectedItem.Text + ")";
                }
                else if (rdSelect.SelectedValue == "1")
                {
                    divProgresschart.Visible = true;
                    divTabular.Visible = false;
                    heading.InnerHtml = "Monthly Physical Progress Chart - " + DDlProject.SelectedItem.Text + " (" + DDLWorkPackage.SelectedItem.Text + ")";
                    LoadGraph();
                }
        ...
        private void LoadTabularData()
        {
            ...
            //foreach (DataRow gvr in getdt.GetTaskScheduleDatesforGraph(new Guid(DDLWorkPackage.SelectedValue)).Tables[0].Rows)
            //{
            //    dr = dt.NewRow();
            ...
            foreach (DataRow gvr in getdt.GetTaskScheduleDatesforGraph(new Guid(DDLWorkPackage.SelectedValue)).Tables[0].Rows)
            {
                dr = dt.NewRow();
                dsvalues.Clear();
                dsvalues = getdt.GetTaskScheduleValuesForGraph(new Guid(DDLWorkPackage.SelectedValue), Convert.ToDateTime(gvr["StartDate"].ToString()), Convert.ToDateTime(gvr["StartDate"].ToString()).AddMonths(1));
                if (dsvalues.Tables[0].Rows.Count > 0)
                {
                    planvalue = decimal.Parse(dsvalues.Tables[0].Rows[0]["TotalSchValue"].ToString());
                    actualvalue = decimal.Parse(dsvalues.Tables[0].Rows[0]["TotalAchValue"].ToString());
                ...
                dr["Months"] = Convert.ToDateTime(gvr["StartDate"].ToString()).ToString("MMM-yy");
                dr["ProjectedPer"] = decimal.Round(planvalue, 2);
                dr["AchievedPer"] = decimal.Round(actualvalue, 2);
        ...
        protected void GrdPhysicalData_DataBound(object sender, EventArgs e)
        {
            try
            {
                if (ViewState["export"].ToString() == "2")
                {
                    ...
                    cell.ColumnSpan = 2;
                    cell.Text = "Monthwise Weighted Average Progress in %";
                    row.Controls.Add(cell);

                    cell = new TableHeaderCell();
                    cell.ColumnSpan = 2;
                    cell.Text = "Cumulative Weighted Average Progress in %";
                    row.Controls.Add(cell);

                    //row.BackColor = ColorTranslator.FromHtml("#3AC0F2");
                    GrdPhysicalData.HeaderRow.Parent.Controls.AddAt(0, row);
                }
            }
            catch (Exception ex)
            {
        ...
        private void PDFConvert(string isPrint)
        {
            ...
            gdRp.Columns[1].HeaderText = "Projected (%)";
            gdRp.Columns[2].HeaderText = "Achieved (%)";
            gdRp.Columns[3].HeaderText = "Cumulative Projected (%)";
            gdRp.Columns[4].HeaderText = "Cumulative Achieved (%)";
            ...
            Phrase phApplicationName = new Phrase(Environment.NewLine + WebConfigurationManager.AppSettings["Domain"] + " Report Name: Monthly Physical Progress Report", FontFactory.GetFont("Arial", ApplicationNameSize, iTextSharp.text.Font.BOLD));
